...
### Slice 9.7: CI startup volume ownership
...
- Root cause evidence: the approved pinned image manifest declares runtime user `mattermost` and image-owned volumes at `/mattermost/config`, `/mattermost/data`, `/mattermost/logs`, `/mattermost/plugins`, and `/mattermost/client/plugins`. The Compose file additionally mounted a fresh named volume at undeclared `/mattermost/bleve-indexes`, which did not inherit an image directory owned by the unprivileged runtime user and prevented readiness.
- RED command: `pnpm exec vitest run src/contracts/mattermost/run.test.ts -t 'image-owned Mattermost data volume'` — failed because the parser still required the standalone `mattermost-bleve` volume (`Mattermost contract Compose volumes must match the disposable topology`).
- GREEN command/result: the same focus passed after moving `MM_BLEVESETTINGS_INDEXDIR` to `/mattermost/data/bleve-indexes`, removing the standalone Bleve mount/volume, and tightening the exact topology to the six remaining project-owned volumes. `src/contracts/mattermost/run.test.ts` passes 15/15; `pnpm test:mattermost:safety` passes 5 files/66 tests.
- Affected verification: `pnpm typecheck`, `git diff --check`, Compose `config --quiet`, and validation of the actual normalized JSON passed; `pnpm test` passes 62 files/799 tests.
- REFACTOR performed: the operator guide now records why Bleve belongs under the image-owned data volume. No privileged init container, root runtime, bind mount, custom volume driver, emulation, image substitution, credential logging, or weakened readiness/mutation assertion was introduced.
