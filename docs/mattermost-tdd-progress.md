...
- Diagnostic result: [run 29142450371](https://github.com/ufJmacca/nanoclaw/actions/runs/29142450371) proved Postgres healthy and Mattermost running without fatal server errors, but Mattermost had no published host port (`Publishers: []`). The child failed only because all 120 loopback readiness attempts were unreachable. The image's unrelated `mmctl --local` health probe is also unhealthy because local mode is deliberately disabled; the live harness does not use or enable that privileged local-admin path.

### Slice 9.9: internal-network server access through a host loopback proxy

- Reproduction: a disposable local container on an `internal: true` Compose network normalized the configured published port to `PublishedPort: 0`; `curl http://127.0.0.1:18065` failed, while the host could reach the same service directly at its private `172.20.0.2` bridge address. The diagnostic project and network were removed immediately afterward.
- Orchestration RED command: `pnpm exec vitest run src/contracts/mattermost/run.test.ts -t 'validates Compose, proves'` — the actual command list omitted Compose-owned container discovery and Docker network inspection, so the injected loopback proxy factory was never called and never closed.
- Orchestration GREEN: startup now obtains the exact 64-hex container ID from `docker compose ... ps -q mattermost`, inspects exactly `${projectName}_contract`, accepts only one RFC1918 IPv4 address, and opens an exclusive host proxy from `127.0.0.1:8065` to container port 8065. Stop destroys tracked sockets and closes the proxy before authoritative `down --volumes`; a proxy-close error cannot skip Compose cleanup.
- Proxy RED command: `pnpm exec vitest run src/contracts/mattermost/run.test.ts -t 'binds the real contract proxy'` — the runtime proxy was not exported (`expected Any<Function>, received undefined`). An initial real-listener Green attempt was rejected as fixture evidence because the local sandbox forbids socket binding; the fixture was corrected to inject only network primitives without skipping behavior.
- Proxy GREEN: the corrected focus passes while asserting the real implementation binds exclusively to `127.0.0.1:8065`, connects only to the validated private target on 8065, tracks both directions, destroys them on close, and closes its server. Compose now declares no published ports at all and retains its single internal, non-external, project-scoped network.
