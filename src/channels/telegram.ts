...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const pairedUserId = consumed.consumed!.adminUserId ? `telegram:${consumed.consumed!.adminUserId}` : null;
      if (pairedUserId) {
        upsertUser({
          id: pairedUserId,
          kind: 'telegram',
        ...
        log.warn('Telegram pairing accepted without an author user id', { platformId });
      }
